...
const Footer = () => {
  return (
    <footer className="flex place-content-center">
      <div className="flex flex-col items-center justify-center py-6 px-10 gap-4 bg-dark w-full">
        ...
        <div className="flex flex-row justify-center gap-10 flex-wrap">
          {socials.map((item) => (
            <a key={item.title} href={item.href} target="_blank" rel="noreferrer">
              <div key={item.title} className="px-2 py-1 rounded-lg bg-light">
                <Image src={item.icon} width={24} height={24} alt={item.title} className="rounded social-icon" />
              </div>
            </a>
          ))}
        </div>
